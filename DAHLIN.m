...
disp('PROJETO DE CONTROLADOR DE DAHLIN');
...
if n == 1
   if d == 1
      Rz = conv(Bz,[1 -1]); % Polinômio R(z^-1)
      Sz = conv(Az,(1-alfa)); % Polinômio S(z^-1)
      Tz = Sz; % Polinômio T(z^-1)
   elseif d == 2
      Rz = conv(Bz,[1 -alfa -(1-alfa)]); % Polinômio R(z^-1)
      Sz = conv(Az,(1-alfa)); % Polinômio S(z^-1)
      Tz = Sz; % Polinômio T(z^-1)
   else
      Rz = conv(Bz,[1 -alfa zeros(1,d-2) -(1-alfa)]); % Polinômio R(z^-1)
      Sz = conv(Az,(1-alfa)); % Polinômio S(z^-1)
      Tz = Sz; % Polinômio T(z^-1)
   end
...
disp('FIM DO PROJETO DE CONTROLADOR DE DAHLIN');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (19/05/2019)