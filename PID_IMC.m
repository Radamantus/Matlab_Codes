%% INÍCIO DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (14/05/2019)
...
disp('PROJETO DE CONTROLADOR PID VIA IMC');
...
delay = d-1; % Atraso de transporte em número de amostras
bf = 1-zd; af = zd; % Parâmetros da sintonia IMC
K = bf/(sum(Bz)+(1+delay*(1-af))); % Parâmetro único

% Projeto analógico (IMC - Internal Model Tuning for PID)
kc = K*(1-a2); % Ganho proporcional
ki = K*(1+a1+a2); % Ganho integral
kd = K*a2; % Ganho derivativo
...
if m == 1 && o == 1
% Caso PID ideal
s0 = kc*(1+Td); s1 = kc*(-1+(1/Ti)-(2*Td)); s2 = Td*kc; % Mapeamento do plano s para o plano z (backward)
elseif m == 2 && o == 1
% Caso PID paralelo
s0 = kc+kd; s1 = -kc+ki-2*kd; s2 = kd; % Mapeamento do plano s para o plano z (backward)
elseif m == 1 && o == 2
% Caso PID ideal
...
disp('FIM DO PROJETO DE CONTROLADOR PID VIA IMC');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (14/05/2019)