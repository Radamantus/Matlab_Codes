...
disp('PROJETO DE CONTROLADOR UHPC');
...
for i = 1:d-1
    SOMA(i) = Ca*PHIa^(Ny-i)*Ga;
    m3 = SOMA;
end

% Memórias da lei de controle
m0 = 1/(lambda+Ca*H);
m1 = Ca*PHIa^Ny-Ca*F*Ca;
m2 = Ca*F;
...
for k = na+d+Ny+1:nit 
    ...
    % Lei de controle UHPC
       if Ny == 1
       du(k) = m0*(yr(k)-m1*x(:,k)-m2*yv(k)); % Incremento de controle
       else
     % du(k) = m0*(yr(k+d)-m1*x(:,k)-m2*yv(k)-m3*fliplr(du(k-1:-1:k-d+1))'); % Incremento de controle
       du(k) = m0*(yr(k+d)-m1*x(:,k)-m2*yv(k)-m3*du(k-d+1:k-1)'); % Incremento de controle
       end
...
disp('FIM DO PROJETO DE CONTROLADOR UHPC');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (22/01/2019)