...
disp('PROJETO DE CONTROLADOR PID');
...
if n == 1
   disp('[1] - PID digital, [2] - Pseudo PID digital ou [3] - PID paralelo:'); % Selecionar PID a ser sintonizado
...
elseif m == 2
% Projeto Pseudo PID digital (IMC - Internal Model Tuning for Pseudo PID)
kc = (tau_ma^2+2*csi*tau_ma+Ts^2)/(3.6*kp*(teta+tau_mf)*Ts);
s0 = 3.6*kc; s1 = -6*kc; s2 = 2.5*kc; t0 = 0.1*kc;
Sz = [s0 s1 s2]; % Polinômio S(z^-1)
Tz = t0; % Polinômio T(z^-1)
elseif m == 3
...
disp('FIM DO PROJETO DE CONTROLADOR PID');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (09/05/2016)