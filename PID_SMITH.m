...
disp('PROJETO DE CONTROLADOR PID COM PREDITOR DE SMITH');
...
disp('[1] - Malha fechada com PID ou [2] - Malha aberta:'); % Opções de malha de controle
...
teta = d*Ts; % Atraso de transporte (delay) em segundos
...
y1 = zeros(1,nit); % Inicializar vetor de sinal de saída auxiliar 1
y2 = zeros(1,nit); % Inicializar vetor de sinal de saída auxiliar 2
e1 = zeros(1,nit); % Inicializar vetor de sinal de erro auxiliar 1
e2 = zeros(1,nit); % Inicializar vetor de sinal de erro auxiliar 2
...
for k = na+d+1:nit 
    ...
    % Saída do modelo sem atraso
    y1(k) = -Az(2:length(Az))*y1(k-1:-1:k-na)' ...
          +Bz*uv(k-1:-1:k-nb)';
    % Saída do modelo com atraso
    y2(k) = -Az(2:length(Az))*y2(k-1:-1:k-na)' ...
          +Bz*uv(k-d:-1:k-nb+1-d)';
      
    % Sinal de erro entre saída da planta e saída do modelo
    e1(k) = yv(k)-y2(k);
    
    % Somatório entre o erro entre planta e modelo mais saída do modelo sem atraso
    e2(k) = e1(k)+y1(k);
    ...
    if n == 1
       if o == 1
       % Lei de controle PID
       u(k) = u(k-1)+Tz*yr(k:-1:k-nt+1)'-Sz*e2(k:-1:k-ns+1)'; % Sinal de controle
       elseif o == 2
       % Lei de controle PID
       u(k) = u(k-2)+Tz*yr(k:-1:k-nt+1)'-Sz*e2(k:-1:k-ns+1)'; % Sinal de controle
       end
    uv(k) = u(k);
...
disp('FIM DO PROJETO DE CONTROLADOR COM PREDITOR DE SMITH');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (09/05/2016)