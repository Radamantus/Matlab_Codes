%% INÍCIO DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (09/04/2018)
...
%% Obter controlador LQG na forma canônica RST
z = tf('z',Ts); % Transformada Z
sys = ss(PHI,G,C,D,Ts); % Sistema aumentado em espaço de estados
plant = tf(sys); % Sistema aumentado em função de transferência
M = PHIa-L*Ca*PHIa-Ga*K+L*Ca*Ga*K; % Matriz de transição de estados do controlador
sys = ss(M,L,K,D,Ts); % Controlador em espaço de estados
controller = tf(sys)*(z^1/(z-1)); % Controlador em função de transferência
closedloop = feedback(series(controller,plant),1); % Controlador + planta em malha fechada
Dz = [1 -1]; % Delta = 1 - z^-1
Sz = controller.num{1}; % Polinômio S(z^-1) na forma: Sz = [s0 s1 s2 ... sn]
Tz = Sz; % Polinômio T(z^-1) na forma: Tz = [t0 t1 t2 ... tn]
Rz = deconv(controller.den{1},Dz); % Polinômio R(z^-1) na forma: Rz = [1 r1 r2 ... rn]
