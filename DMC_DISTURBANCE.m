...
Sf = zeros(Np,Nc); % Inicializar a matriz Sf (Movimentos futuros)
Sp = zeros(Np,Nm-2); % Inicializar a matriz Sp (Movimentos passados)

for i = 2:Nc
    G(:,i) = [zeros(i-1,1); G(1:Np-i+1,1)]; % Matriz de Toeplitz
end
  for j = 1:Nc
      Sf(:,j) = [zeros(j-1,1); gi(1:Np-j+1)];
  end

  for i = 1:Np
      Sp(i,:) = [gi(i+1:Nm-1)' zeros(1,i-1)];
  end
...
yp = zeros(1,nit); % Inicializar vetor de sinal de saída predita
p = zeros(1,nit); % Inicializar vetor de sinal de perturbação
% gi(end) é a última amostra (coeficiente da resposta ao degrau)
% gi(1) é a primeira amostra (coeficiente da resposta ao degrau)
...
for k = na+d+1:nit-w
    ...
    for m0 = 1:Np
        ...
        if k-Nm+1 > 0 % Predição do modelo
           yp(k+1) = gi(1)*du(k) + Sp(1,:)*duf(1:Nm-2)'+gi(end)*u(k-Nm+1);
        else
           yp(k+1) = gi(1)*du(k) + Sp(1,:)*duf(1:Nm-2)';
        end
        
    p(k) = yv(k)-yp(k); % Compensação da perturbação
    ef = yr(k+w)*ones(Np,1)-f+p(k)*ones(Np,1); % Erros futuros
