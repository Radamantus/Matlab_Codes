...
disp('PROJETO DE CONTROLADOR MPC');
...
% Kmpc = [Kdx; Ky];
% p = eig(PHIa-Ga*Kmpc); % Polos de malha fechada
...
xf = zeros(length(PHIa),nit); % Inicializar vetor de estados aumentado
...
for k = length(PHI)+d+1:nit
    ...
    % Estados aumentados
    dx(:,k) = x(:,k)-x(:,k-1); % Variação dos estados nominais
    xf(:,k) = [dx(:,k); yv(k)]; % % Variação dos estados nominais e saída da planta
    
    if n == 1
    % Lei de controle MPC
    % DU = (AA+rw*eye(Nc,Nc))\(AR*yr(k)-AF*xf(:,k));
    % du(k) = DU(1,1); % Incremento de controle
    du(k) = -Kmpc*xf(:,k)+Kmpc(end)*yr(k); % Incremento de controle
...
disp('FIM DO PROJETO DE CONTROLADOR MPC');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (15/02/2018)