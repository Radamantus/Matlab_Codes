%% INÍCIO DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (27/01/2019)
...
disp('PROJETO DE CONTROLADOR GMVSS FILTRADO NA FORMA RST');
...
% Az = [1 -1.756766263148975 0.778838481671449];
% Bz = [0.021873312969509 0.000196423745169];
% Cz = [1 -0.833138066713129 0.000534870620086];
...
Ez = [Pz(1) Ez]; % Polinômio F(z^-1) na forma: Ez = [e0 e1 e2 ... en]
Fz = F'; % Polinômio F(z^-1) na forma: Fz = [f0 f1 f2 ... fn]
BEz = conv(PBz,Ez); % Polinômio PB(z^-1)*E(z^-1)
lamCz = lambda*Cz; % Polinômio PC(z^-1)*lambda
...
Tz = Pz; % Polinômio T(z^-1) a partir da lei de controle GMVSS
...
disp('FIM DO PROJETO DE CONTROLADOR GMVSS FILTRADO NA FORMA RST');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (27/01/2019)