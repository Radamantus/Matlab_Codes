%% INÍCIO DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (09/10/2017)
...
disp('PROJETO DE CONTROLADOR PID ESTOCÁSTICO');
...
PHIa = rot90(PHIa,2)'; % PHIa = flipud(fliplr(PHIa))'; Matriz de transição de estados aumentados discreta
Ga = fliplr(Ca)'; % Matriz de entrada aumentada discreta
Ca = flipud(Ba)'; % Matriz de saída  aumentada discreta
...
% PHIa = rot90(PHIa,2); % PHIa = flipud(fliplr(PHIa)); Matriz de transição de estados aumentados discreta
% Ga = flipud(Ba); % Matriz de entrada aumentada discreta
% Ca = fliplr(Ca); % Matriz de saída aumentada discreta

% T = [c(n)-a(n) c(n-1)-a(n-1) c(n-2)-a(n-2) . . . c(1)-a(1)];
% Observação: utilizar os polinômios A(z^-1) e C(z^-1) aumentados
T = flipud(Cbar(2:length(Cbar))'-Abar(2:length(Abar))'); % Inicializar o vetor Gamma
...
for k = na+d+1:nit 
    ...
    % Filtro de Kalman
    xa(:,k) = (PHIa-T*Ca)*xa(:,k-1)+Ga*du(k-1)+T*yv(k-1);
    yf(k) = Ca*xa(:,k);
    eest(k) = y(k)-yf(k);
    u(k) = u(k-1)+(1/lambda)*(Tz*yr(k:-1:k-nt+1)'-Sz*yf(k:-1:k-ns+1)'); % Sinal de controle
    uv(k) = u(k); 
...
disp('FIM DO PROJETO DE CONTROLADOR PID ESTOCÁSTICO');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (09/10/2017)