...
G = zeros(Ny-d,Nu); % Inicializar a matriz G
g = zeros(1,Ny-d); % Inicializar o vetor g
Azbarra = conv(Az,[1 -1]); % Polinômio A(z^-1)*(1-z^-1)
Atil = [Azbarra zeros(1,Ny-d)];
abarra = Atil(2:end);
natil = length(Azbarra)-1;

% Matriz G
aux = Bz;
auxb = [aux zeros(1,Ny-d)];
g(1) = auxb(1);
...
for k = na+d+1:nit 
    ...
    ent_fut = 0;
    if ent_fut == 0
       aux_ref = yr(k)*ones(1,Ny-d)'; % Referência futura desconhecida
    elseif ent_fut == 1
           aux_ref = yr(k:k+Ny-d-1)'; % Referência futura conhecida
    end
    
    ye = aux_ref-yf(k:k+Ny-d-1)';
