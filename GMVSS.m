...
disp('PROJETO DE CONTROLADOR GMVSS');
...
SOMA = zeros(1,dgmv); % Inicializar lado esquerdo da lei de controle

for i = 1:dgmv
    SOMA(i) = Ca*PHIa^(dgmv-i)*Ga;
    SOMA(dgmv) = SOMA(dgmv)+lambda;
    m3 = SOMA(1:length(SOMA)-1);
end

% Memórias da lei de controle
m0 = 1/SOMA(dgmv);
m1 = Ca*PHIa^dgmv-Ca*F*Ca;
m2 = Ca*F;
...
for k = na+d+dgmv+1:nit 
    ...
    % Lei de controle GMVSS
       if dgmv == 1
       du(k) = m0*(yr(k+d)-m1*x(:,k)-m2*yv(k)); % Incremento de controle
       else
     % du(k) = m0*(yr(k+d)-m1*x(:,k)-m2*yv(k)-m3*fliplr(du(k-1:-1:k-dgmv+1))'); % Incremento de controle
...
disp('FIM DO PROJETO DE CONTROLADOR GMVSS');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (04/10/2016)