%% INÍCIO DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (20/05/2019)
...
disp('PROJETO DE CONTROLADOR DE VOGEL-EDGAR');
...
disp('[1] - Malha fechada com controlador de Vogel-Edgar ou [2] - Malha aberta:'); % Opções de malha de controle
...
%% Projeto do controlador de Vogel-Edgar
...
if n == 1
   Mz = conv(sum(Bz),[1 -alfa]); % Polinômio M(z^-1)
   Nz = conv((1-alfa),Bz); % Polinômio N(z^-1)
   Nz = [zeros(1,d) Nz]; % Polinômio N(z^-1)z^-d
   if na == 1 && d == 1
      Rz = Mz-Nz; % Polinômio R(z^-1)
   else
      Mz = [Mz zeros(1,length(Nz)-2)];
      Rz = Mz-Nz; % Polinômio R(z^-1)
   end
   Sz = conv((1-alfa),Az); % Polinômio Sz
   Tz = Sz; % Polinômio Tz
...
disp('FIM DO PROJETO DE CONTROLADOR DE VOGEL-EDGAR');
%% FIM DA ROTINA
%% LUÍS AUGUSTO MESQUITA DE CASTRO (20/05/2019)